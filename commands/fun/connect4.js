...
this.run = async (client, msg, args) => {
	if(Sessions.has(msg.channel.id)) return msg.channel.send('This channel already playing connect4');
	const wait = require('util').promisify(setTimeout);
	try{
		let user = msg.mentions.users.first();
		if(!user) user = client.user;
		if(!user.bot){
			const verifyMessage = await msg.channel.send(`${user.toString()}, Are you accept this challenge?`);
			const isVerified = await verify(verifyMessage, user);
...
async function verify(msg, user){
	await msg.react('🇾');
	await msg.react('🇳');
	const filter = (rect, usr) => ['🇾', '🇳'].includes(rect.emoji.name) && usr.id === user.id;
	const response = await msg.awaitReactions(filter, { max: 1, time: 15000});
	if(!response.size){
		await msg.delete();
		await msg.channel.send('Look like they declined');
		return false;
	}
	if(response.first().emoji.name === '🇾') return true;
	await msg.delete();
	await msg.channel.send('Look like they declined');
	return false;
}
...
exports.info = {
	name: 'connect4',
	description: 'play connect4 of another user',
	usage: 'connect4 [user]',
	aliases: ['c4'],
	ownerOnly: false,
	authorPerm: [],
